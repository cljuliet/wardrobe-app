...
const STORAGE_WISHLIST = "wp7-wishlist";
...
function useStore(key, seed, userId) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .catch(() => {
        setSynced(true); // unblock writes even if read fails
      });
  }, [userId, key]);
...
function generateOutfits(pieces) {
  ...
  DAYS.forEach((day, i) => {
    ...
    if (shoe) useCount(shoe.id);

    const out = shoe ? [bottom.id, top.id, shoe.id] : [bottom.id, top.id];
...
// ║  MOBILE VIEW                                                      ║
// ╚══════════════════════════════════════════════════════════════════╝
function MobileApp({ pieces, setPieces, plan, setPlan, customBrands, setCustomBrands, wishlist, setWishlist, signOut }) {
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100vh",display:"flex",flexDirection:"column",color:C.text,overflow:"hidden" }}>
      ...
      <div style={{ background:C.appBar,padding:"12px 16px 10px",flexShrink:0,paddingTop:"max(12px,env(safe-area-inset-top))" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex",gap:6,alignItems:"center" }}>
            ...
            <button onClick={signOut} style={{ background:"none",border:"none",color:"rgba(240,237,230,0.5)",fontSize:10,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",padding:"2px 6px" }}>Sign out</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ background:C.tabBar,display:"flex",alignItems:"stretch",flexShrink:0,paddingBottom:"env(safe-area-inset-bottom)",borderTop:`1px solid #333` }}>
        {[
          ...
          { id:"wishlist", label:"Wishlist", icon: (active) => {
            const s = active ? C.tabActive : C.tabInactive;
            return (
              <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                <path d="M11 18.5s-8-5-8-10a5 5 0 0110 0 5 5 0 0110 0c0 5-8 10-8 10z" stroke={s} strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M11 8.5v5M8.5 11h5" stroke={s} strokeWidth="1.3" strokeLinecap="round"/>
              </svg>
            );
          }},
        ].map(t=>(
      ...
      {tab==="wishlist" && (
        <div style={{ position:"fixed",inset:0,background:C.bg,zIndex:200,display:"flex",flexDirection:"column",paddingBottom:"env(safe-area-inset-bottom)" }}>
          <WishlistPanel wishlist={wishlist} setWishlist={setWishlist} setPieces={setPieces} onClose={null}/>
        </div>
      )}
...
// ║  DESKTOP VIEW                                                     ║
// ╚══════════════════════════════════════════════════════════════════╝
function DesktopApp({ pieces, setPieces, plan, setPlan, customBrands, setCustomBrands, wishlist, setWishlist, signOut }) {
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100vh",display:"flex",flexDirection:"column",fontSize:14,color:C.text,overflow:"hidden" }}>
      ...
      {/* Wishlist side panel */}
      {showWishlist && (
        <div style={{ position:"fixed",top:0,right:0,bottom:0,width:420,background:C.bg,boxShadow:"-4px 0 32px rgba(0,0,0,0.12)",zIndex:400,display:"flex",flexDirection:"column",borderLeft:`1px solid ${C.border}` }}>
          <WishlistPanel wishlist={wishlist} setWishlist={setWishlist} setPieces={setPieces} onClose={()=>setShowWishlist(false)}/>
        </div>
      )}
    </div>
  );
...
// ║  WISHLIST COMPONENTS                                              ║
// ╚══════════════════════════════════════════════════════════════════╝
const WISH_EMPTY = { name:"", brand:"", category:"top", price:"", link:"", priority:false, notes:"" };
const WISH_CAT = ["top","bottom","layer","shoes","bag","accessory","other"];

function BudgetBar({ spent, budget }) {
  const pct = budget > 0 ? Math.min((spent / budget) * 100, 100) : 0;
  const over = spent > budget && budget > 0;
  const color = over ? C.danger : pct > 80 ? "#B07A2E" : C.purchased;
  return (
    <div>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:6 }}>
        <span style={{ fontSize:11,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",color:C.textMuted }}>Budget</span>
        <span style={{ fontSize:13,color:over?C.danger:C.text,fontWeight:600 }}>
          ${spent.toLocaleString()} <span style={{ color:C.textMuted,fontWeight:400 }}>/ ${budget>0?budget.toLocaleString():"—"}</span>
        </span>
      </div>
      {budget > 0 && (
      ...
      {over && <div style={{ fontSize:11,color:C.danger,marginTop:4 }}>Over budget by ${(spent-budget).toLocaleString()}</div>}
    </div>
  );
}

function WishItem({ item, onDelete, onTogglePriority, onMoveToLibrary }) {
  const [hov, setHov] = useState(false);
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ background:hov?C.bgCardHov:C.bgCard,border:`1.5px solid ${item.priority?C.accent:C.borderLight}`,borderRadius:12,padding:"14px 16px",marginBottom:8,transition:"all .12s",position:"relative" }}>
      <div style={{ display:"flex",alignItems:"flex-start",gap:12 }}>
        <div style={{ width:8,height:8,borderRadius:"50%",background:CAT_DOT[item.category]||C.textMuted,flexShrink:0,marginTop:5 }}/>
        <div style={{ flex:1,minWidth:0 }}>
          <div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:2,flexWrap:"wrap" }}>
            <span style={{ fontSize:14,fontWeight:600,color:C.text }}>{item.name}</span>
            {item.priority && <span style={{ fontSize:9,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",color:C.accent,background:C.bgHeader,padding:"2px 6px",borderRadius:99 }}>Priority</span>}
          </div>
          <div style={{ fontSize:12,color:C.textMuted,marginBottom:4 }}>{item.brand}{item.brand&&item.category?" · ":""}{item.category}</div>
          {item.notes && <div style={{ fontSize:11,color:C.textLight,fontStyle:"italic",marginBottom:4 }}>"{item.notes}"</div>}
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            <span style={{ fontSize:15,fontWeight:600,color:C.text }}>${Number(item.price).toLocaleString()}</span>
            {item.link && <a href={item.link} target="_blank" rel="noreferrer" style={{ fontSize:11,color:C.accentSoft,textDecoration:"none" }}>Shop ↗</a>}
          </div>
        </div>
        {hov && (
          <div style={{ display:"flex",flexDirection:"column",gap:4,flexShrink:0 }}>
            <button onClick={()=>onTogglePriority(item.id)}
              style={{ background:item.priority?C.accent:C.bgHeader,color:item.priority?"#F0EDE6":C.textMid,border:"none",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:10,fontWeight:600,fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap" }}>
              {item.priority?"★ Priority":"☆ Priority"}
            </button>
            <button onClick={()=>onMoveToLibrary(item.id)}
              style={{ background:C.purchasedBg,color:C.purchased,border:"none",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:10,fontWeight:600,fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap" }}>
              → Library
            </button>
            <button onClick={()=>onDelete(item.id)}
              style={{ background:C.dangerBg,color:C.danger,border:"none",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:10,fontWeight:600,fontFamily:"'DM Sans',sans-serif" }}>
              Delete
            </button>
          </div>
        )}
...
function WishAddForm({ onAdd, onCancel }) {
  const [form, setForm] = useState(WISH_EMPTY);
  const set = (k,v) => setForm(f=>({...f,[k]:v}));
  const valid = form.name.trim() && form.price && !isNaN(parseFloat(form.price));
  const iStyle = { width:"100%",border:`1.5px solid ${C.border}`,borderRadius:8,padding:"9px 12px",fontSize:13,fontFamily:"'DM Sans',sans-serif",color:C.text,outline:"none",background:C.bg,boxSizing:"border-box" };
  return (
    <div style={{ background:C.bgCard,border:`1.5px solid ${C.accent}`,borderRadius:12,padding:"18px 16px",marginBottom:12 }}>
      <div style={{ fontSize:11,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",color:C.textMuted,marginBottom:12 }}>Add to wishlist</div>
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8 }}>
        <input value={form.name} onChange={e=>set("name",e.target.value)} placeholder="Item name" style={iStyle}/>
        <input value={form.brand} onChange={e=>set("brand",e.target.value)} placeholder="Brand" style={iStyle}/>
        <input value={form.price} onChange={e=>set("price",e.target.value)} placeholder="Price $" type="number" style={iStyle}/>
        <select value={form.category} onChange={e=>set("category",e.target.value)} style={{...iStyle,background:C.bg}}>
          {WISH_CAT.map(c=><option key={c} value={c}>{c.charAt(0).toUpperCase()+c.slice(1)}</option>)}
        </select>
      </div>
      <input value={form.link} onChange={e=>set("link",e.target.value)} placeholder="Link (optional)" style={{...iStyle,marginBottom:8}}/>
      <input value={form.notes} onChange={e=>set("notes",e.target.value)} placeholder="Notes (optional)" style={{...iStyle,marginBottom:12}}/>
      <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:12 }}>
        <div onClick={()=>set("priority",!form.priority)}
          style={{ width:16,height:16,borderRadius:4,border:`2px solid ${form.priority?C.accent:C.border}`,background:form.priority?C.accent:C.bgCard,cursor:"pointer",flexShrink:0 }}/>
        <span style={{ fontSize:12,color:C.textMid,cursor:"pointer" }} onClick={()=>set("priority",!form.priority)}>Mark as priority</span>
      </div>
      <div style={{ display:"flex",gap:8 }}>
        <button onClick={()=>{ if(!valid) return; onAdd({...form,id:`w${Date.now()}`,price:parseFloat(form.price)}); }}
          disabled={!valid}
          style={{ background:valid?C.appBar:C.border,color:"#F0EDE6",border:"none",borderRadius:8,padding:"9px 20px",cursor:valid?"pointer":"default",fontSize:13,fontWeight:600,fontFamily:"'DM Sans',sans-serif" }}>
          Add to wishlist
        </button>
        <button onClick={onCancel}
          style={{ background:"none",border:`1.5px solid ${C.border}`,borderRadius:8,padding:"9px 16px",cursor:"pointer",fontSize:13,color:C.textMid,fontFamily:"'DM Sans',sans-serif" }}>
          Cancel
        </button>
      </div>
    </div>
...
function WishlistPanel({ wishlist, setWishlist, setPieces, onClose }) {
  ...
  const totalAll      = wishlist.reduce((s,i) => s+Number(i.price), 0);
  const totalPriority = wishlist.filter(i=>i.priority).reduce((s,i) => s+Number(i.price), 0);
  const shown = filter==="priority" ? wishlist.filter(i=>i.priority) : wishlist;

  const addItem    = (item) => { setWishlist(prev=>[item,...prev]); setShowAdd(false); };
  const deleteItem = (id)   => setWishlist(prev=>prev.filter(i=>i.id!==id));
  const togglePri  = (id)   => setWishlist(prev=>prev.map(i=>i.id===id?{...i,priority:!i.priority}:i));
  const moveToLib  = (id)   => {
    const item = wishlist.find(i=>i.id===id);
    if (item) {
      const libItem = { id:`c${Date.now()}`,name:item.name,brand:item.brand||"",variant:"",price:Number(item.price),link:item.link||"",image:"",category:item.category,purchased:false };
      setPieces(prev=>[...prev,libItem]);
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",height:"100%",overflow:"hidden" }}>
      ...
      <div style={{ padding:"14px 20px 12px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0 }}>
        <div style={{ fontFamily:"'DM Serif Display',serif",fontStyle:"italic",fontSize:18,color:C.text }}>Wishlist</div>
        <div style={{ display:"flex",gap:8,alignItems:"center" }}>
          <button onClick={()=>setShowAdd(s=>!s)}
            style={{ background:C.appBar,color:"#F0EDE6",border:"none",borderRadius:6,padding:"6px 14px",cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"'DM Sans',sans-serif" }}>
            + Add item
          </button>
          {onClose && <button onClick={onClose} style={{ background:"none",border:"none",cursor:"pointer",color:C.textMuted,fontSize:20,lineHeight:1,padding:"0 4px" }}>×</button>}
        </div>
      </div>

      <div style={{ flex:1,overflowY:"auto",padding:"16px 20px 24px" }}>
        {/* Budget card */}
        <div style={{ background:C.bgCard,border:`1.5px solid ${C.borderLight}`,borderRadius:14,padding:"16px 18px",marginBottom:14 }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}>
            <span style={{ fontSize:13,fontWeight:600,color:C.text }}>Budget tracker</span>
            {!editingBudget
              ? <button onClick={()=>{ setEditingBudget(true); setBudgetInput(String(budget)); }}
                  style={{ background:"none",border:`1px solid ${C.border}`,borderRadius:6,padding:"3px 8px",cursor:"pointer",fontSize:11,color:C.textMid,fontFamily:"'DM Sans',sans-serif" }}>Edit</button>
              : <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                  <input value={budgetInput} onChange={e=>setBudgetInput(e.target.value)} type="number"
                    style={{ width:80,border:`1.5px solid ${C.accent}`,borderRadius:6,padding:"3px 8px",fontSize:12,fontFamily:"'DM Sans',sans-serif",color:C.text,outline:"none",background:C.bg }}/>
                  <button onClick={()=>{ setBudget(parseFloat(budgetInput)||0); setEditingBudget(false); }}
                    style={{ background:C.appBar,color:"#F0EDE6",border:"none",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:11,fontWeight:600,fontFamily:"'DM Sans',sans-serif" }}>Save</button>
                </div>
            }
          </div>
          <BudgetBar spent={totalAll} budget={budget}/>
          <div style={{ display:"flex",gap:8,marginTop:12 }}>
            <div style={{ flex:1,background:C.bgHeader,borderRadius:8,padding:"8px 10px" }}>
              <div style={{ fontSize:9,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",color:C.textMuted,marginBottom:2 }}>All items</div>
              <div style={{ fontSize:14,fontWeight:600,color:C.text }}>${totalAll.toLocaleString()}</div>
              <div style={{ fontSize:10,color:C.textMuted }}>{wishlist.length} piece{wishlist.length!==1?"s":""}</div>
            </div>
            <div style={{ flex:1,background:C.bgHeader,borderRadius:8,padding:"8px 10px" }}>
              <div style={{ fontSize:9,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",color:C.textMuted,marginBottom:2 }}>Priority</div>
              <div style={{ fontSize:14,fontWeight:600,color:C.accent }}>${totalPriority.toLocaleString()}</div>
              <div style={{ fontSize:10,color:C.textMuted }}>{wishlist.filter(i=>i.priority).length} piece{wishlist.filter(i=>i.priority).length!==1?"s":""}</div>
            </div>
            {budget > 0 && (
              <div style={{ flex:1,background:C.bgHeader,borderRadius:8,padding:"8px 10px" }}>
                <div style={{ fontSize:9,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",color:C.textMuted,marginBottom:2 }}>Remaining</div>
                <div style={{ fontSize:14,fontWeight:600,color:budget-totalAll>=0?C.purchased:C.danger }}>${Math.abs(budget-totalAll).toLocaleString()}</div>
                <div style={{ fontSize:10,color:C.textMuted }}>{budget-totalAll>=0?"left":"over"}</div>
              </div>
            )}
          </div>
        </div>

        {showAdd && <WishAddForm onAdd={addItem} onCancel={()=>setShowAdd(false)}/>}

        {/* Filter */}
        <div style={{ display:"flex",gap:6,marginBottom:10 }}>
          {[["all","All"],["priority","Priority"]].map(([val,label])=>(
            <button key={val} onClick={()=>setFilter(val)}
              style={{ fontSize:11,padding:"4px 12px",borderRadius:99,border:`1.5px solid ${filter===val?C.accent:C.border}`,background:filter===val?C.accent:"transparent",color:filter===val?"#F0EDE6":C.textMid,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:500 }}>
              {label}
            </button>
          ))}
        </div>
        ...
        {shown.length === 0 && (
          <div style={{ textAlign:"center",padding:"40px 20px",color:C.textMuted }}>
            <div style={{ fontFamily:"'DM Serif Display',serif",fontStyle:"italic",fontSize:17,marginBottom:4 }}>
              {filter==="priority"?"No priority items yet":"Your wishlist is empty"}
            </div>
            <div style={{ fontSize:12 }}>Add pieces you are considering buying</div>
          </div>
        )}

        {shown.map(item => (
          <WishItem key={item.id} item={item}
            onDelete={deleteItem} onTogglePriority={togglePri} onMoveToLibrary={moveToLib}/>
        ))}
      </div>
    </div>
  );
...
function AppInner() {
  ...
  // Listen for auth state — handles magic link callback via URL hash/code
  useEffect(() => {
    // onAuthStateChange fires immediately with current session,
    // AND fires again when the magic link hash/code is exchanged for a session
  ...
  const [wishlist, setWishlist]         = useStore(STORAGE_WISHLIST, [],          userId);
  ...
  const props = { pieces, setPieces, plan, setPlan, customBrands, setCustomBrands, wishlist, setWishlist, signOut };
