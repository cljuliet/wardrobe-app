...
// Multiple safeguards prevent seed/empty data from ever overwriting real Supabase data.
function useStore(key, seed, userId) {
  ...
  const [readOk, setReadOk]           = useState(false); // true only if Supabase read succeeded
  const [userModified, setUserModified] = useState(false);
  const [remoteCount, setRemoteCount] = useState(0); // item count in Supabase

  const setValUser = (updater) => {
    setUserModified(true);
    setVal(updater);
  };

  // Safeguard: is this value just the seed data unchanged?
  const isSeedData = (v) => {
    try { return JSON.stringify(v) === JSON.stringify(seed); } catch { return false; }
  };

  // Safeguard: would this write reduce item count vs what Supabase has?
  const wouldReduceData = (v) => {
    if (!Array.isArray(v)) return false;
    return v.length < remoteCount;
  };

  // On login: ALWAYS read from Supabase first. If read fails, block all writes.
  useEffect(() => {
    if (!userId) { setSynced(false); setReadOk(false); setUserModified(false); setRemoteCount(0); return; }
    supabase
      .from("wardrobe_data")
      .select("value")
      .eq("user_id", userId)
      ...
      .then(({ data, error }) => {
        if (data?.value !== undefined) {
          ...
          // Track how many items exist in Supabase
          if (Array.isArray(remoteVal)) setRemoteCount(remoteVal.length);
          // Always prefer Supabase data over local cache
          setVal(remoteVal);
          try { localStorage.setItem(key, JSON.stringify(remoteVal)); } catch {}
          setReadOk(true);
        } else {
          // No data in Supabase yet — safe to write when user makes changes
          setReadOk(true);
        }
        setSynced(true);
      })
      .catch(() => {
        // Read failed — block all writes this session to be safe
        console.warn("[useStore] Supabase read failed for key:", key, "— writes blocked");
        setReadOk(false);
        setSynced(true);
      });
  }, [userId, key]);

  // On change: multiple checks before writing to Supabase
  useEffect(() => {
    try { localStorage.setItem(key, JSON.stringify(val)); } catch {}

    // Block write if: not logged in, not synced, read failed, or user never touched data
    if (!userId || !synced || !readOk || !userModified) return;

    // Block write if val is literally the seed data
    if (isSeedData(val)) {
      console.warn("[useStore] blocked write of seed data for key:", key);
      return;
    }
    ...
    supabase.from("wardrobe_data").upsert(
      ...
      else if (Array.isArray(val)) setRemoteCount(val.length); // keep count in sync
    });
  }, [val, userId, synced, readOk, userModified, key]);

  return [val, setValUser];
}
    ...
    <div style={{ fontFamily:"'DM Sans',sans-serif",background:C.bg,height:"100vh",display:"flex",flexDirection:"column",fontSize:14,color:C.text,overflow:"hidden" }}>
      ...
      {showWishlist && (
        <div style={{ position:"fixed",top:0,right:0,bottom:0,width:420,background:C.bg,boxShadow:"-4px 0 32px rgba(0,0,0,0.12)",zIndex:400,display:"flex",flexDirection:"column",borderLeft:`1px solid ${C.border}` }}>
          <WishlistPanel pieces={pieces} onClose={()=>setShowWishlist(false)}/>
        </div>
      )}
    </div>
  );
...
function WishlistPanel({ pieces, onClose }) {
  const [budget, setBudget]           = useState(2000);
  const [editingBudget, setEditingBudget] = useState(false);
  const [budgetInput, setBudgetInput] = useState("2000");

  const unpurchased = pieces.filter(p => !p.purchased);
  const total = unpurchased.reduce((s,p) => s + Number(p.price), 0);
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",height:"100%",overflow:"hidden" }}>
      <div style={{ padding:"14px 20px 12px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0 }}>
        ...
        {onClose && <button onClick={onClose} style={{ background:"none",border:"none",cursor:"pointer",color:C.textMuted,fontSize:20,lineHeight:1,padding:"0 4px" }}>x</button>}
      </div>
      ...
      <div style={{ flex:1,overflowY:"auto",padding:"16px 20px 24px" }}>
        <div style={{ background:C.bgCard,border:`1.5px solid ${C.borderLight}`,borderRadius:14,padding:"16px 18px",marginBottom:14 }}>
          ...
          <div style={{ display:"flex",gap:8,marginTop:12 }}>
            <div style={{ flex:1,background:C.bgHeader,borderRadius:8,padding:"8px 10px" }}>
              <div style={{ fontSize:9,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",color:C.textMuted,marginBottom:2 }}>To buy</div>
              <div style={{ fontSize:14,fontWeight:600,color:C.text }}>${total.toLocaleString()}</div>
              <div style={{ fontSize:10,color:C.textMuted }}>{unpurchased.length} piece{unpurchased.length!==1?"s":""}</div>
            </div>
            <div style={{ flex:1,background:C.bgHeader,borderRadius:8,padding:"8px 10px" }}>
              <div style={{ fontSize:9,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",color:C.textMuted,marginBottom:2 }}>Purchased</div>
              <div style={{ fontSize:14,fontWeight:600,color:C.purchased }}>{pieces.filter(p=>p.purchased).length} piece{pieces.filter(p=>p.purchased).length!==1?"s":""}</div>
              <div style={{ fontSize:10,color:C.textMuted }}>${pieces.filter(p=>p.purchased).reduce((s,p)=>s+Number(p.price),0).toLocaleString()} spent</div>
            </div>
            {budget > 0 && (
              <div style={{ flex:1,background:C.bgHeader,borderRadius:8,padding:"8px 10px" }}>
                ...
                <div style={{ fontSize:14,fontWeight:600,color:budget-total>=0?C.purchased:C.danger }}>${Math.abs(budget-total).toLocaleString()}</div>
                <div style={{ fontSize:10,color:C.textMuted }}>{budget-total>=0?"left":"over"}</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ fontSize:11,fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",color:C.textMuted,marginBottom:8 }}>
          {unpurchased.length} unpurchased piece{unpurchased.length!==1?"s":""}
        </div>

        {unpurchased.length === 0 && (
          <div style={{ textAlign:"center",padding:"40px 20px",color:C.textMuted }}>
            <div style={{ fontFamily:"'DM Serif Display',serif",fontStyle:"italic",fontSize:17,marginBottom:4 }}>All caught up!</div>
            <div style={{ fontSize:12 }}>Every piece in your library is purchased.</div>
          </div>
        )}

        {unpurchased.map(p => (
          <div key={p.id} style={{ background:C.bgCard,border:`1.5px solid ${C.borderLight}`,borderRadius:12,padding:"12px 14px",marginBottom:8,display:"flex",gap:12,alignItems:"center" }}>
            <div style={{ width:8,height:8,borderRadius:"50%",background:CAT_DOT[p.category]||C.textMuted,flexShrink:0 }}/>
            <div style={{ flex:1,minWidth:0 }}>
              <div style={{ fontSize:13,fontWeight:600,color:C.text,marginBottom:1 }}>{p.name}{p.variant ? ` - ${p.variant}` : ""}</div>
              <div style={{ fontSize:11,color:C.textMuted }}>{p.brand} · {CAT[p.category]||p.category}</div>
            </div>
            <div style={{ display:"flex",alignItems:"center",gap:8 }}>
              <span style={{ fontSize:14,fontWeight:600,color:C.text }}>${Number(p.price).toLocaleString()}</span>
              {p.link && <a href={p.link} target="_blank" rel="noreferrer" style={{ fontSize:11,color:C.accentSoft,textDecoration:"none" }}>Shop</a>}
            </div>
          </div>
        ))}
      </div>
